...
# Funções de cálculo de índices e classificação
...
def classify_ndwi_and_ndvi(ndvi, ndwi):
...
# Função para salvar e mostrar imagens
...
classified = classify_ndwi_and_ndvi(ndvi, ndwi)

# Visualização
full_palette_with_water = ['gray'] + [NDVI_CLASSES[i]['cor'] for i in NDVI_CLASSES] + ['blue']
urls = {
    "RGB da composição": composite.select(['B4', 'B3', 'B2']).visualize(min=0, max=3000).getThumbURL({
        'region': ROIs[0], 'dimensions': '512x512', 'format': 'png'
    }),
    "NDVI contínuo": ndvi.visualize(min=-1, max=1, palette=['blue', 'white', 'green']).getThumbURL({
        'region': ROIs[0], 'dimensions': '512x512', 'format': 'png'
    }),
    "NDWI contínuo": ndwi.visualize(min=-1, max=1, palette=['white', 'blue']).getThumbURL({
        'region': ROIs[0], 'dimensions': '512x512', 'format': 'png'
    }),
    "Mapa NDVI e NDWI classificado": classified.visualize(min=-1, max=6, palette=full_palette_with_water).getThumbURL({
        'region': ROIs[0], 'dimensions': '512x512', 'format': 'png'
    })
}
